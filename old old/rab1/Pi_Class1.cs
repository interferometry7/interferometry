...
// ----------------------------------------------------------------------------------------------------------------------------------------------------------------  
        ...
        // -----------------------------------------------------------------------------------------------------------------------------------
        static void Z_bmp(ProgressBar progressBar1, Bitmap bmp, Int32[,] Z, int w, int h)               // -------------------------- Z -> BMP
        {
...
// --------------------------------------------------------------------------------------------------------------------------------                
// --------------------------------------------------------------------------------------------------------------------------------
        public static void pi2_frml(Image[] img, PictureBox pictureBox01, ProgressBar progressBar1, string sN1, string sN2, int Diag, Point p, int x0_end, int x1_end, int y0_end, int y1_end,int rb, int pr_obr)
        {
            China(sN1, sN2);           // Вычисление формулы
            Graph_China(img, pictureBox01, progressBar1, Diag, p,  x0_end, x1_end, y0_end, y1_end, rb, pr_obr);                          // Построение таблицы
        }
...
//-----------------------------------------------------------------------------------------------------------------------------------
        ...
        public static void pi2_rshfr(Image[] img, PictureBox pictureBox01, ProgressBar progressBar1, int xx0, int xx1, int yy0, int yy1, string sN1, string sN2, int Diag) // Расшифровка
        {
            China(sN1, sN2);                                           // Вычисление формулы 
            int w = img[0].Width;
            int h = img[0].Height;
            ...
            Bitmap bmp = new Bitmap(pictureBox01.Image, w, h);
            Z = new int[w, h];
            rash_2pi(progressBar1,  bmp, bmp1, bmp2, w, h,  Diag);     //  РАСШИФРОВКА (Заполнение Z[,])
            Z_bmp(progressBar1, bmp, Z, w, h);                  //  Z -> bmp с масштабированием
            pictureBox01.Size = new System.Drawing.Size(w, h);
            pictureBox01.Image = bmp;
        }

        public static void pi2_ABC( PictureBox pictureBox01, ProgressBar progressBar1, int xx0, int xx1, int yy0, int yy1) // Устранение тренда по методу наименьших квадратов
        {
 ...
 // ------------------------------------------------------------------------------------------------------------------- Вычитание плоскости, проходящей через 3 точки
        public static void NKL(PictureBox pictureBox01, ProgressBar progressBar1, int x1, int y1, int x2, int y2, int x3, int y3)
        {   
